...
All six implementation steps are complete locally. Verification passed 23 unit tests and 24 desktop/mobile browser scenarios. Actual results and publication limits are recorded in [the development log](docs/development-log.md). The existing Apache-2.0 license is preserved.
...
- **Four visual layers:** synthetic ocean temperature, symbolic sea-level/coastal change, shrinking glacier-mass symbols, and a cyclone-formation environment experiment.
...
3. Select **태풍** (Cyclone). Scroll to its experiment and increase wind shear or set the latitude to zero. Watch the environment score and spiral respond.
...
All timing starts paused. At 2100, playing again restarts from 2020. Changing a numeric slider pauses playback. Hiding the browser tab, opening a dialog or changing views stops playback. Shared links include settings, not personal checklist state or camera orientation.
