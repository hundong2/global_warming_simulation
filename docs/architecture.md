...
- `src/render/globe.ts`: local geography loading, generated map texture, globe, grid, symbolic glaciers/cyclone, camera, resize, context recovery, and disposal.
- `src/ui/content.ts`: Korean lessons, attribution links, action cards, and authored SVG icons.
- `src/main.ts`: owns the settings, coordinates views and dialogs, runs discrete year playback, updates DOM output, and exports settings.
...
All shared values are allowlisted and normalized. The hash carries the scenario, target, year, ENSO phase, layer, shear, humidity, latitude, and adaptation. Localhost links work only where that local server is reachable; the dialog explains this. JSON export includes a schema version, settings, evaluated values, assumptions, and source links. No personal information is collected.
